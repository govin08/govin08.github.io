...
2차원 평면 위의 $n$개의 점들 $X_1$, $\cdots$ $X_n$을 생각하자.
이 $n$개의 점들을 가장 잘 설명할 수 있는 직선을 그어보자.
'잘 설명할 수 있다'는 것의 의미는 
...
이다.
(a)는 다음 그림에서 빨간 선들의 제곱의 평균이 최소가 되는 경우를 뜻한다.

![1-2]({{site.url}}\images\2025-12-31-pca\1-2.png){: .img-50-center}

일반성을 잃지 않고 $n$개의 점들의 무게중심이 원점이라고 하자.
원점을 지나는 직선은 방향벡터로 결정되므로, (a)조건을 만족시키는 방향벡터 $v$를 구하는 문제가 된다.
...
참고로, [Pattern Recognition and Machine Learning](https://tjzhifei.github.io/links/PRML.pdf)의 12.1.1, 12.1.2를 보면 이것보다 더 일반적인 경우에 대한 equivalence를 증명하고 있다.


# 1. Preliminaries

## 1.1 dataset, variance, covariance
