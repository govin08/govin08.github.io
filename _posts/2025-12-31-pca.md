...
$m$개의 feature들을 일차결합하여 새로운 성분 $Xv_1$을 만들되, 의미있는 성분을 만들어보려 한다 (단, $v\in\mathbb R^m$).
...
<div class="notice--info">
$\text{var}(Xv_1)$을 최대화하는 $v_1$에 대하여 벡터 $Xv_1$을 $X$의 첫번째 주성분(the first principal component)라고 부른다.
</div>
...
<div class="notice--info">
$v_2\perp v_1$인 조건을 만족하면서 $\text{var}(Xv_2)$을 최대화하는 $v_2$에 대하여 벡터 $Xv_2$을 $X$의 두번째 주성분(the second principal component)라고 부른다.
</div>

$k$번째 주성분이 구해졌을 때, $k+1$번째 주성분은 다음과 같이 정한다.

<div class="notice--info">
$v_{k+1}\perp v_1,\cdots,v_{k+1}\perp v_k$인 조건을 만족하면서 $\text{var}(Xv_{k+1})$을 최대화하는 $v_{k+1}$에 대하여 벡터 $Xv_{k+1}$을 $X$의 $k+1$번째 주성분(the $k+1$th principal component)라고 부른다.
</div>
