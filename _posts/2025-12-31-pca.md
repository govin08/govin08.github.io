...
<div class="notice--info">
$v_2\perp v_1$인 조건을 만족하면서 $\text{var}(Xv_2)$을 최대화하는 단위벡터 $v_2$에 대하여 벡터 $Xv_2$을 $X$의 두번째 주성분(the second principal component)라고 부른다.
</div>

$k$번째 주성분이 구해졌을 때, $k+1$번째 주성분은 다음과 같이 정한다.

<div class="notice--info">
$v_{k+1}\perp v_1,\cdots,v_{k+1}\perp v_k$인 조건을 만족하면서 $\text{var}(Xv_{k+1})$을 최대화하는 단위벡터 $v_{k+1}$에 대하여 벡터 $Xv_{k+1}$을 $X$의 $k+1$번째 주성분(the $k+1$th principal component)라고 부른다.
</div>
...
따라서 ${v_1}^Tv_1=1$를 만족시키면서 ${v_1}^TCv_1$을 최대화해야 한다.
이 최적화 문제에 대한 lagrangian은

$$\mathcal L(v_1,\lambda_1)={v_1}^TCv_1+\lambda_1(1-{v_1}^Tv_1)$$
